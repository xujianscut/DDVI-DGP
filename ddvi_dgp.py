...
class LSDE(nn.Module):
    def __init__(self, num_inducing, device, t1=0.1):
        super().__init__()
        ...
        self.sigma = nn.Parameter(torch.ones(1, num_inducing, device=device))
...
class SDEVariationalDistribution(gpytorch.variational._variational_distribution._VariationalDistribution):
    def __init__(self, num_inducing_points, device, batch_shape=torch.Size([]), mean_init_std=1e-3, t1=0.1):
        ...
        # SDE components
        self.u0 = nn.Parameter(torch.randn(1, num_inducing_points, device=device))
        self.usde = LSDE(num_inducing=num_inducing_points, device=device, t1=t1)
        ...
        self.covar_init = torch.eye(num_inducing_points, device=device)  # prior p(U)
        self.mean_init = self.mean_init.repeat(*batch_shape, 1)
    ...
    def u(self):
        ...
        t0 = 0.0
        dt = 1e-3
        steps = max(2, int((t1 - t0) / dt) + 1)
        ts = torch.linspace(t0, t1, steps=steps, device=self.u0.device)
        u_s = torchsde.sdeint(self.usde, y0=self.u0, ts=ts, names={'drift': 'f', 'diffusion': 'g'},
                              method='euler', dt=dt)
        u = u_s[-1, :, :]
        return u.squeeze()
...
def main():
    ...
    parser.add_argument('--t1', type=float, default=1e-3, help='Terminal time for SDE')
    parser.add_argument('--dt', type=float, default=1e-3, help='Euler step for SDE integration')
    parser.add_argument('--loss_sde_hidden1', type=float, default=1e-3, help='Weight for hidden layer SDE loss')
    parser.add_argument('--loss_sde_hidden2', type=float, default=1e-3, help='Weight for hidden layer SDE loss')
    parser.add_argument('--loss_sde_hidden3', type=float, default=1e-3, help='Weight for hidden layer SDE loss')
    parser.add_argument('--loss_sde_hidden4', type=float, default=1e-3, help='Weight for hidden layer SDE loss')
    
    parser.add_argument('--loss_sde_last', type=float, default=1e3, help='Weight for last layer SDE loss (0=off)')
    ...
    for _ in epochs_iter:
        ...
        for x_batch, y_batch in minibatch_iter:
            ...
            with gpytorch.settings.num_likelihood_samples(args.num_samples):
                ...
                # 公共参数
                t0 = 0.0
                steps = max(2, int((args.t1 - t0) / args.dt) + 1)
                ts = torch.linspace(t0, args.t1, steps=steps, device=device)

                sde_losses = []
                for i, hl in enumerate(model.hidden_layers):
                    u0 = hl.variational_distribution.u0
                    g = hl.variational_distribution.usde.sigma.unsqueeze(0)**2
                    kappa_vals = hl.variational_distribution.usde.kappa(ts, dt=args.dt)

                    u_s = torchsde.sdeint(
                        hl.variational_distribution.usde,
                        y0=u0, ts=ts,
                        names={'drift': 'f', 'diffusion': 'g'},
                        method='euler', dt=args.dt
                    )
                    r = hl.variational_distribution.usde.nn1(u_s)
                    sde_loss = 0.5 * torch.norm(g * (u_s/kappa_vals + r))**2
                    sde_losses.append(sde_loss)

                # last layer
                u0 = model.last_layer.variational_distribution.u0
                g = model.last_layer.variational_distribution.usde.sigma.unsqueeze(0)**2
                kappa_vals = model.last_layer.variational_distribution.usde.kappa(ts, dt=args.dt)

                u_s = torchsde.sdeint(
                    model.last_layer.variational_distribution.usde,
                    y0=u0, ts=ts,
                    names={'drift': 'f', 'diffusion': 'g'},
                    method='euler', dt=args.dt
                )
                r = model.last_layer.variational_distribution.usde.nn1(u_s)
                last_loss = 0.5 * torch.norm(g * (u_s/kappa_vals + r))**2

                # 总 loss
                loss = loss1
                for j, sde_loss in enumerate(sde_losses, start=1):
                    loss += getattr(args, f"loss_sde_hidden{j}") * sde_loss
                loss += args.loss_sde_last * last_loss
